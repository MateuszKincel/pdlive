    ...
    if($_POST["action"] == 'fetch')
    {
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '<button type="button"   name="view_button" class="btn btn-info btn-circle view_button" data-id="'.$row["promotor_id"].'" title="Podgląd tematów"><i class="fa-sharp fa-solid fa-eye"></i></button> 
							<button type="button"   name="edit_button" class="btn btn-warning btn-circle edit_button" promotor_id="'.$row["promotor_id"].'" title="Edytuj liczbę tematów."><i class="fa-sharp fa-solid fa-cog"></i></button>';
...
if($_POST["action"] == 'fetch_single')
{
    ...
    echo json_encode($data);
		...
		if ($_POST["action"] == 'edit_single') {
			...
			$object->query = "SELECT * FROM promotor WHERE promotor_id = '$promotor_id'";
